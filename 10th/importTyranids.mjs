...
newTyranids.category.cards.map((card, index) => {
  const foundNid = oldTyranids.datasheets.findIndex((oldCard) => oldCard.name === card.name);
...
fs.writeFileSync(path.resolve(__dirname, `gdc/tyranids.json`), JSON.stringify(sortObj(oldTyranids), null, 2));
